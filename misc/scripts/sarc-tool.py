    ...
    for path, dirs, files in os.walk(root):
        ...
        for file in files:
            ...
            for folder in filename.split("/")[:-1]:
                if not i:
                    folders_dict[i] = SarcLib.Folder(folder + '/')
                    arc.addFolder(folders_dict[i])
                else:
                    folders_dict[i] = SarcLib.Folder(folder + '/')
                    folders_dict[i - 1].addFolder(folders_dict[i])
            ...
            if not i:
                ...
                folders_dict[i-1].addFile(SarcLib.File(file, inb, hasFilename))
